...
//   api/[category]/[action].js   -> /api/{id}/full|episodes            (auto-detect)
//   api/mal/[id]/[action].js     -> /api/mal/{mal_id}/full|episodes    (forced source)
...
const { handleFull, handleEpisodes } = require("./media");
...
module.exports = async function itemRoute(req, res, source) {
  ...
  if (!source && (idRaw === "mal" || idRaw === "anilist")) {
    return jsonResponse(res, 400, {
      ...
      hint:
        `Explicit-source routes look like /api/${idRaw}/{id}/full and /api/${idRaw}/{id}/episodes ` +
  ...
  return jsonResponse(res, 404, {
    ...
    hint:
      `Try /api/${source ? `${source}/{id}` : "{id}"}/full for full anime metadata or ` +
      `/api/${source ? `${source}/{id}` : "{id}"}/episodes for the episode list. ` +
